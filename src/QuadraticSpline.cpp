...
QuadraticSpline::QuadraticSpline(Model* model) :
Polygon(model) {};

QuadraticSpline::QuadraticSpline(const Component& copy) :
Polygon(copy) {};
 ...
 */
void QuadraticSpline::draw_internal() const {
  std::vector<Point>* curve = new std::vector<Point>();
  quadratic_spline(*object_, HIGH_SMOOTHNESS, curve);
 ...
 */
void QuadraticSpline::draw_selected(const Point* selected) const  {
 ...
 */
Point* QuadraticSpline::on_object(const unsigned int& x,
  ...
  if(selected) { // if selected only check if on control polygon
    ...
    std::vector<Point>* curve = new std::vector<Point>();
    quadratic_spline(*object_, LOW_SMOOTHNESS, curve);// calculate the curve
                                                      // with low polygon count
 ...
 */
bool QuadraticSpline::is_smooth(const std::vector<Point>& part,
 ...
 */
void QuadraticSpline::quadratic_spline( const std::vector<Point>& c_points,
  ...
  if(is_smooth(c_points, smoothness)) {
    ...
    sub_points->push_back(c_points.at(0) * 1/2 + c_points.at(1) * 1/2);

    for(int i = 0, i_end = c_points.size() - 1; i < i_end; ++i) {
      sub_points->push_back(c_points.at(i) * 3/4 + c_points.at(i+1) * 1/4);
      if(i < i_end - 1) {
        sub_points->push_back(c_points.at(i) * 1/4 + c_points.at(i+1) * 3/4);
      }
    }

    sub_points->push_back(*(c_points.end()-1));

    quadratic_spline(*sub_points, smoothness, curve);
